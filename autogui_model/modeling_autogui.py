...
class QWenAttention(nn.Module):
    def __init__(self, config):
        ...
        if use_flash_attention:
            print(Fore.CYAN + "Using flash attention for QwenAttention" + Style.RESET_ALL)
    ...
    def forward(
        ...
        if self.training and use_flash_attention:
...
class AutoGUIModel(QWenPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.use_ref_tag = getattr(config, "use_ref_tag", True) # This flag is used by the AutoGUI dataset. use_ref_tag=False means the "<ref>" tag in dialogs will be deleted.
    ...
    def set_grad_checkpointing(self, enable=True):
        if self.config.visual["image_size"] == 448:
            print(Fore.YELLOW + "Disable ViT gradient checkpointing as this experiment uses 448x448" + Style.RESET_ALL)
            enable = False
    ...
    def forward(
        ...
        if past_key_values is None and torch.any(input_ids == self.config.visual['image_start_id']):
            ...
            try:
                ...
                raise e
            assert not self.use_ref_tag or (len(points_feature) if points_feature is not None else 0 == len(ground_pos))
...
class AutoGUILMHeadModel(QWenPreTrainedModel):
    ...
    def get_point(self, tokenizer, sentence):
        ...
        regex = r"<ref>\s*\( <coord (\d+)> ,\s*? <coord (\d+)> \)" if ' <coord ' in sentence else r"<ref>\s*\((\d+),\s*(\d+)\)"
        for x, y in re.findall(regex, sentence):
