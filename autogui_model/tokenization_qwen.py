...
### support multi-digit
DIGIT_NUMBER = 2
PAT_STR = r"""(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\r\n\p{L}\p{N}]?\p{L}+|\p{N}{1,<<digit_number>>}| ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+""".replace('<<digit_number>>', str(DIGIT_NUMBER))
### end
ENDOFTEXT = "<|endoftext|>"
IMSTART = "<|im_start|>"
IMEND = "<|im_end|>"
        ...
        **kwargs,
        ...
        self.special_tokens = {
            ...
            for index, token in enumerate(
                SPECIAL_TOKENS + self.IMAGE_ST, start= 151643 # It must be start at 151643 because the original version does not support adding new tokens.
            )
        }
        ...
        ### seems no need to check enc.n_vocab??? 
        # assert (
